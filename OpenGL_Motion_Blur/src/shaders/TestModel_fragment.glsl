...
void main(void)
{
	...
	light[0].ambient	=light[1].ambient	= vec3(1f, 1f, 1f);
	light[0].diffuse	=light[1].diffuse	= vec3(0.8f, 0.8f, 0.8f);
	light[0].specular	=light[1].specular	= vec3(0.5f, 0.5f, 0.5f);

	vec3 result = vec3(0,0,0);
	for(int i =0; i<2; i++)
		result += calcPointLight(light[i], normal, vs_fragCoord,  viewDir);

	color = texture(u_texture, vs_textureCoord) * vec4(result, 1);
}